...
CASOS = [
    ...
    Caso("el cableado por defecto deja de pintar", INDEX,
         "           sellarReloj: saveMeta, activos: hasRealLocalData, pintar: setSyncUI };",
         "           sellarReloj: saveMeta, activos: hasRealLocalData, pintar: () => {} };",
    ...
    Caso("la bajada deja de distinguir un documento inutilizable", INDEX,
         "  if (decision.soltar || !decision.utilizable) {",
         "  if (decision.soltar) {",
    ...
    Caso("PERMANENTE-3: un documento inutilizable deja de soltar el freno", INDEX,
         "    d.soltarFreno();\n    if (decision.soltar) d.sellarReloj();",
         "    if (decision.soltar) d.sellarReloj();",
    ...
    # ── 01-09 · el recibo de aterrizaje (D-74). Cada caso nace del mensaje con el
    # que MURIÓ al aplicarlo a mano sobre una copia, no de memoria. ──
    Caso("01-09 R01: el arranque vuelve a \u00abla ausencia suelta\u00bb (s\u00f3lo pendiente)", INDEX,
         "    const frena = !p.hayRecibo || conPendiente;",
         "    const frena = conPendiente;",
         1, "01-09 AC-1 tras la RECARGA el freno sigue puesto"),
    Caso("01-09 R02: el recibo se ESCRIBE al empezar a aterrizar", INDEX,
         "  if (!quitarRecibo()) {",
         "  if (!escribirMarca(RECIBO_KEY)) {",
         1, "01-09 AC-1 pero el recibo S\u00cd se quit\u00f3"),
    Caso("01-09 R03: el recibo no se quita antes de aterrizar", INDEX,
         "  if (!quitarRecibo()) {\n    armarFreno(data, [RECIBO_KEY]);\n    return false;\n  }\n",
         "",
         1, "01-09 AC-1 pero el recibo S\u00cd se quit\u00f3"),
    Caso("01-09 R04: META corrupta suelta (primitivos sin formato)", INDEX,
         "    catch (e) { return { hayFormato, hayRecibo, meta: 'ilegible', pendiente: null }; }",
         "    catch (e) { return { hayFormato: false, hayRecibo, meta: 'ilegible', pendiente: null }; }",
         1, "01-09 AC-2 con la META corrupta el freno sigue puesto"),
    Caso("01-09 R05: con formato se mira la META", INDEX,
         "    const frena = !p.hayRecibo || conPendiente;",
         "    const frena = p.meta !== 'ilegible' && (!p.hayRecibo || conPendiente);",
         1, "01-09 AC-2 con la META corrupta el freno sigue puesto"),
    Caso("01-09 R06: (iii) sin la condici\u00f3n > 0", INDEX,
         "  return remoto > 0 && (Number(savedAtLocal) || 0) === remoto;",
         "  return (Number(savedAtLocal) || 0) === remoto;",
         1, "01-09 AC-4iii vacuidad: una marca cero no prueba nada"),
    Caso("01-09 R07: (iii) la bajada del arranque ignora soltar", INDEX,
         "  if (decision.soltar || !decision.utilizable) {",
         "  if (!decision.utilizable) {",
         1, "01-09 AC-4iii el documento ya en disco suelta el freno"),
    Caso("01-09 R08: (iii) pinta verde", INDEX,
         "      return { aplicar: false, utilizable: true, soltar: true, aviso: 'pendiente',",
         "      return { aplicar: false, utilizable: true, soltar: true, aviso: 'ok',",
         1, "01-09 AC-4iii con aviso naranja, no verde"),
    Caso("01-09 R09: la siembra escribe el formato antes que el recibo", INDEX,
         "  return { freno: null, sembrar: [RECIBO_KEY, FORMATO_KEY] };",
         "  return { freno: null, sembrar: [FORMATO_KEY, RECIBO_KEY] };",
         1, "01-09 AC-5 formato=false recibo=true meta=ausente: siembra"),
    Caso("01-09 R10: sin formato se frena siempre", INDEX,
         "  return { freno: null, sembrar: [RECIBO_KEY, FORMATO_KEY] };",
         "  return { freno: cifras, sembrar: [RECIBO_KEY, FORMATO_KEY] };",
         1, "01-09 AC-5 formato=false recibo=true meta=ausente: frena"),
    Caso("01-09 R11: el arranque pisa el freno armado", INDEX,
         "  if (!libroPendiente) {\n    const r = frenoAlArrancar(primitivosDelFreno());",
         "  if (true) {\n    const r = frenoAlArrancar(primitivosDelFreno());",
         1, "01-09 AC-5 el arranque no pisa un freno ya armado"),
    Caso("01-09 R12: la siembra sigue tras fallar", INDEX,
         "    if (!escribirMarca(clave)) return false;",
         "    escribirMarca(clave);",
         1, "01-09 AC-5 si el recibo no cabe, el formato NO se escribe"),
    Caso("01-09 R13: con el recibo fallido no se adopta la memoria", INDEX,
         "  const meta = metaDelDocumento(data);\n  portfolios    = meta.portfolios;",
         "  const meta = metaDelDocumento(data);\n  if (!escribirMarca(RECIBO_KEY)) return false;\n  portfolios    = meta.portfolios;",
         1, "01-09 AC-4iii si s\u00f3lo falla el recibo, el documento S\u00cd se da por aplicado"),
    Caso("01-09 R14: el guardado no repone el recibo", INDEX,
         "  if (todoBien) { reponerRecibo(); schedulePush(); }",
         "  if (todoBien) { schedulePush(); }",
         1, "01-09 AC-4iii el siguiente guardado repone el recibo"),
    # 01-09 R15 RETIRADO en la revision del diff: su ancla ya no existe porque
    # reponerRecibo ya no mira el freno sino la DEUDA de su pestaña (H3). Lo cubre H3a.
    Caso("01-09 R16: soltarFreno no repone el recibo", INDEX,
         "  // Nada pendiente de aterrizar: eso es exactamente lo que el recibo afirma.\n  escribirMarca(RECIBO_KEY);",
         "",
    ...
    Caso("01-09 R18: sin formato, pendiente no frena", INDEX,
         "  if (conPendiente) return { freno: cifras, sembrar: [FORMATO_KEY] };",
         "",
         1, "AC-1 y al recargar se recupera"),
    Caso("01-09 R19: el freno deducido inventa cifras", INDEX,
         "  if (f.deducido) return MOTIVO_FRENO_DEDUCIDO;",
         "",
         1, "01-09 AC-1 y su motivo no inventa cifras"),
    # ── 01-09 · D-67: el empate al dia y el aviso PROPIO del freno. ──
    Caso("01-09 E01: el empate vuelve a nube-no-mas-nueva", INDEX,
         "  if (remoto === localSaved) {\n    return { aplicar: false, utilizable: true, aviso: 'ok', clave: 'al-dia',",
         "  if (false) {\n    return { aplicar: false, utilizable: true, aviso: 'ok', clave: 'al-dia',",
         1, "01-09 AC-6 y est\u00e1 al d\u00eda, no pendiente"),
    Caso("01-09 E02: el empate APLICA", INDEX,
         "    return { aplicar: false, utilizable: true, aviso: 'ok', clave: 'al-dia',",
         "    return { aplicar: true, utilizable: true, aviso: 'ok', clave: 'al-dia',",
         1, "AC-1 un empate NO se aplica"),
    Caso("01-09 E03: la desigualdad de aplicar pasa a >=", INDEX,
         "  if (remoto > localSaved) {",
         "  if (remoto >= localSaved) {",
         1, "AC-1 un empate NO se aplica"),
    Caso("01-09 E04: el freno se pinta pendiente en la SUBIDA", INDEX,
         "    return { subir: false, aviso: 'frenado', clave: 'nube-pendiente',",
         "    return { subir: false, aviso: 'pendiente', clave: 'nube-pendiente',",
         1, "AC-4 matriz vacio/con-todo/activos=true/inc=0/con-freno: aviso"),
    Caso("01-09 E05: el freno se pinta pendiente en la BAJADA del arranque", INDEX,
         "    d.pintar(estadoSync('frenado'), motivo);",
         "    d.pintar(estadoSync('pendiente'), motivo);",
         1, "AC-3 y pinta el aviso PROPIO del freno (01-09, D-67)"),
    Caso("01-09 E06: la bajada del arranque devuelve pendiente con el freno", INDEX,
         "    return { sincronizado: true, aplicado: false, aviso: 'frenado',",
         "    return { sincronizado: true, aplicado: false, aviso: 'pendiente',",
         1, "AC-3 el arranque no se da por sincronizado en verde"),
    Caso("01-09 E07: el freno se pinta pendiente en la ESCUCHA", INDEX,
         "      setSyncUI(estadoSync('frenado'), motivo);",
         "      setSyncUI(estadoSync('pendiente'), motivo);",
         1, "AC-3 la escucha en vivo pinta el ROJO del freno por su valor"),
    Caso("01-09 E08: el mapa cerrado no conoce el freno", INDEX,
         "  if (resultado === 'frenado')   return 'frenado';\n",
         "",
         1, "AC-3 y pinta el aviso PROPIO del freno (01-09, D-67)"),
    Caso("01-09 E09: el freno vuelve al t\u00edtulo de los pendientes", INDEX,
         "    dot.title = 'La nube no cabe en este dispositivo';",
         "    dot.title = 'Cambios sin subir';",
         1, "AC-3 y el t\u00edtulo del freno, no el de los pendientes"),
    Caso("01-09 E10: el freno vuelve al naranja", INDEX,
         "    // \u2014hacer sitio\u2014; el naranja es lo que a prop\u00f3sito no se subi\u00f3 y se arregla solo.\n    dot.style.background = '#e74c3c';",
         "    // \u2014hacer sitio\u2014; el naranja es lo que a prop\u00f3sito no se subi\u00f3 y se arregla solo.\n    dot.style.background = '#e67e22';",
         1, "AC-3 la escucha en vivo pinta el ROJO del freno por su valor"),
    Caso("01-09 E11: el texto del freno vuelve a \u00abCambios sin subir\u00bb", INDEX,
    ...
    Caso("01-09 L01: (iii) la escucha suelta el freno pero NO sube lo tecleado", INDEX,
         "      if (decision.soltar) { saveMeta(); schedulePush(); }",
         "      if (decision.soltar) { saveMeta(); }",
    ...
    # ── 01-09 · REVISION DEL DIFF: brazos A (H1-H3) y B (B10, B14, B23, B30, B52),
    # cada uno visto morir sobre copia tras reproducir el defecto con su prueba. ──
    Caso("01-09 H1a: (iii) en el arranque deja el reloj congelado", INDEX,
         "    if (decision.soltar) d.sellarReloj();\n",
         "",
         1, "01-09 AC-4iii al soltar por (iii) la marca de tiempo AVANZA"),
    Caso("01-09 H1b: (iii) en la escucha deja el reloj congelado", INDEX,
         "      if (decision.soltar) { saveMeta(); schedulePush(); }",
         "      if (decision.soltar) { schedulePush(); }",
         1, "01-09 H1 el eco tras soltar por (iii) NO pinta verde con cambios sin subir"),
    Caso("01-09 H1c: el cableado de la bajada no sella con saveMeta", INDEX,
         "           sellarReloj: saveMeta, activos: hasRealLocalData, pintar: setSyncUI };",
         "           sellarReloj: () => true, activos: hasRealLocalData, pintar: setSyncUI };",
         1, "01-09 AC-8 y al soltar por (iii) sella el reloj con saveMeta"),
    Caso("01-09 H2: con el freno armado al arrancar no se siembra el formato", INDEX,
         "    sembrarContrato([FORMATO_KEY]);\n  }\n  const loaded = initPortfolios();",
         "  }\n  const loaded = initPortfolios();",
         1, "01-09 AC-5 con \u00e9l puesto siembra s\u00f3lo el formato"),
    Caso("01-09 H3a: se repone el recibo sin deuda propia (otra pesta\u00f1a)", INDEX,
         "  if (!reciboDebido) return;\n",
         "",
         1, "01-09 H3 un guardado de OTRA pesta\u00f1a no repone el recibo"),
    Caso("01-09 H3b: la deuda del recibo no caduca al empezar otro aterrizaje", INDEX,
         "  reciboDebido = false;\n  if (!quitarRecibo()) {",
         "  if (!quitarRecibo()) {",
         1, "01-09 H3b tras un aterrizaje fallido, una deuda vieja no repone el recibo"),
    Caso("01-09 H3c: el recibo fallido no deja deuda", INDEX,
         "    reciboDebido = true;\n    console.warn('[SYNC] documento aplicado, pero sin recibo",
         "    console.warn('[SYNC] documento aplicado, pero sin recibo",
         1, "01-09 AC-4iii el siguiente guardado repone el recibo"),
    Caso("01-09 B30: la escucha sube en todo rechazo", INDEX,
         "      if (decision.soltar) { saveMeta(); schedulePush(); }",
         "      if (decision.soltar) { saveMeta(); } schedulePush();",
         1, "01-09 B30 la escucha con al-dia NO programa ninguna subida"),
    Caso("01-09 B14: un recibo que no se puede quitar se da por quitado", INDEX,
         "    console.error('[SYNC] no se pudo quitar el recibo de aterrizaje:', e && e.message);\n    return false;",
         "    console.error('[SYNC] no se pudo quitar el recibo de aterrizaje:', e && e.message);\n    return true;",
         1, "01-09 B14 sin poder quitar el recibo, el documento NO se aplica"),
    Caso("01-09 B52: sin poder quitar el recibo no se arma el freno", INDEX,
         "    armarFreno(data, [RECIBO_KEY]);\n",
         "",
         1, "01-09 B52 y el freno queda puesto"),
    Caso("01-09 B10: el disco ilegible al arrancar falla ABIERTO", INDEX,
         "    return { hayFormato: true, hayRecibo: false, meta: 'ilegible', pendiente: null };",
         "    return { hayFormato: false, hayRecibo: true, meta: 'ilegible', pendiente: null };",
         1, "01-09 B10 con las marcas ilegibles el arranque FRENA"),
    Caso("01-09 B23: la salida (iii) pierde su motivo", INDEX,
         "               motivo: 'el documento de la nube ya estaba en este dispositivo: se suben los cambios' };",
         "               motivo: 'el documento anterior no aterriz\u00f3 entero: se reintenta' };",
         1, "01-09 AC-4iii y dice su motivo"),
    Caso("01-09 RS: la suite no restaura la deuda del recibo", INDEX,
         "    reciboDebido = previo.reciboDebido;\n",
         "",
         1, "01-09 la suite devuelve la deuda del recibo como la encontr\u00f3"),
]
